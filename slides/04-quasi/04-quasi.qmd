...
### [Randomization and Observation]{.gray}
...
- Do individuals who are disadvantaged with respect to social position have worse health than those who are advantaged?

Other kinds of questions that could be asked:

- **Would** individuals who are disadvantaged with respect to social position have better health **if they were to become advantaged**?

- **Would** individuals who are advantaged with respect to social position have worse health **if they were to become disadvantaged**?
...
:::
::: {.column width="75%"}
1. Follow-up of individuals in different social groups for various health outcomes (incidence, mortality, risk factors)

2. Adjustment for various confounders/mediators (are inequalities "explained" by....A, B, C?).
...
:::
::: {.column width="75%"}
We have lots of statistical associations between social exposures and health.
...
In an RCT, treatment/exposure is [assigned]{.red} by the investigator
:::
...
- In observational studies, exposed/unexposed groups [exist]{.red} in the source population and are selected by the investigator.
...
SEP and CVD in Australia. [Many low p-values]{.red}

```{r, fig.height=4, fig.width=7}
knitr::include_graphics(here("images", "beauchamp-t1r.png"))
```
...
:::
::: {.column width="50%"}
The average [change]{.blue} over time in the non-exposed (control) group is [subtracted]{.blue} from the gain over time in the exposed (treatment) group.

These are our two 'differences'.

Double differencing removes biases in second period comparisons between the treatment and control group that could be the result from
...
$\mu_{11}—\mu_{10}$ (pre-post in treated)
...
- Abadie and Gardeazabel (2003) pioneered the synthetic control method to examine the economic impact of terrorism in the Basque country, using other Spanish regions as control groups.
...
::: {.column width="75%"}
```{r}
knitr::include_graphics(here("images", "pre-ca.png"))
```
:::
:::
...
::: {.column width="75%"}
```{r}
knitr::include_graphics(here("images", "synthetic-ca.png"))
```
:::
:::
...
```{r, out.width="70%"}
knitr::include_graphics(here("images", "dk-trans.png"))
```
