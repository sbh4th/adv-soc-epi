...
## Data: European Social Survey Round 11: Sweden
...
This is deliberate. @kessels2013 show that including the ranking variable (or any proxy strongly correlated with it) is **misleading**:
...
A near-zero residual would be a false result — an artefact of circularity, not a sign of a well-specified model.
:::

::: {.column width="50%"}
The large residual in our decomposition is therefore **honest**:

- It captures unexplained income-related variation
- It leaves room for a direct income → smoking pathway
- It does not artificially inflate the "explained" share

The appropriate interpretation is that the covariates in the model account for part of the income gradient in smoking, and the rest remains either directly attributable to income or unexplained by the observed determinants.^[@kessels2013; @erreygers2016]
:::
:::::
...
So the [elasticity of smoking]{.red} (from the previous slide) with respect to education is (0.024 * 0.452 / .073) = 0.148

Now we have the [RCI for education]{.blue} = -0.265

So now we can calculate the contribution of education as:

$$\text{Elasticity}\times RCI_{ed} = 0.148 * -0.265 = -.039$$

#### Thus education accounts for -.039/ -0.203 = 19% of the overall $RCI$
:::
:::::
...
::::: columns
::: {.column width="35%"}
Each contribution is a product of two estimated quantities:^[@odonnell2008; @hosseinpoor2006]
...
knitr::include_graphics(
  here("slides", "03-decomp", "figs", "ess11_decomp_bar.png")
)
```
:::
:::::
