...
### [Concentration Index Decomposition]{.black}
...
- Binge drinking negative since it increases smoking but is more common among higher income.
- Married is positive since it decreases smoking but is more common among higher income.
...
::: {.footnote}
Left:  Right: the gap in group means decomposes into **endowments** (different $\bar{x}$, same coefficients — grey triangle to blue dot) and **coefficients** (same $\bar{x}$, different slopes/intercepts — grey triangle to red dot).
:::
...
## BMI distribution by education (High ed = 23.6  |  Low ed: 25.2)

```{r fig.height=5}
knitr::include_graphics(
  here("slides", "03-decomp", "figs", 
       "ess11_IT_bmi_low_ed_density.png")
)
```
